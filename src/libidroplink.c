...
char *get_id_for_email(char *api_endpoint, char *email, char *password, struct error *err)
{
    ...
    if (curl != NULL) {
        ...
        /* POST fields */
        post_fields = malloc((strlen(email) + strlen(password) + 16 + 1) * sizeof(char));
        if (post_fields == NULL) {
            fprintf(stderr, "malloc() failed\n");
            exit(EXIT_FAILURE);
        }
        ...
        if (s.p != NULL && strlen(s.p) > 0) {
            ...
            if (http_code == 200 && cJSON_GetObjectItem(root, "_id") != NULL) {
...
struct user *get_user(char *api_endpoint, char *user_id, char *token, struct error *err)
{
    ...
    if (curl != NULL) {
        ...
        if (s.p != NULL && strlen(s.p) > 0) {
            ...
            if (http_code == 200 && cJSON_GetObjectItem(root, "email") != NULL) {
                usr->email = strdup(cJSON_GetObjectItem(root, "email")->valuestring);
            } else {
                usr->email = NULL;
            }

            if (http_code == 200 && cJSON_GetObjectItem(root, "creation_date") != NULL) {
                usr->creation_date = strdup(cJSON_GetObjectItem(root, "creation_date")->valuestring);
            } else {
                usr->creation_date = NULL;
            }

            if (http_code == 200 && usr->_id == NULL && usr->email == NULL && usr->creation_date == NULL) {
                if (cJSON_GetObjectItem(root, "message") != NULL) {
                    err->description = strdup(cJSON_GetObjectItem(root, "message")->valuestring);
                    err->version = error_version;
                    err->http_code = http_code;
